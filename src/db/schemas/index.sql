...
VALUES ("Açucar", "Açucar Refinado", 2.99, 4.20, 1, 6, 1),
...
SELECT p.id,
  ...
  p.purchase_price,
  p.price,
  (p.price - p.purchase_price) AS profit,
  c.category,
  b.brand,
  w.Weight,
  p.create_at
...
FROM products;
/* Entrada do 0*/
INSERT INTO inventory (product_id, qtd)
VALUES (1, 20),
  (2, 5),
  (3, 10);
/* Incrementar qtd +1*/
...
FROM exit_history AS e
  INNER JOIN inventory AS i ON e.inventory_id = i.id
  INNER JOIN products AS p ON i.product_id = p.id
